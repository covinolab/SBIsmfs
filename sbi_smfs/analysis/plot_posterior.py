...
from sbi_smfs.utils import gls_spline
from sbi_smfs.utils.config_utils import get_config_parser


def plot_spline_ensemble(posterior_samples, num_splines, config):
...
def plot_spline_mean_with_error(posterior_samples, config, alpha=0.5):
    config = get_config_parser(config)
    if "Dx" in config["SIMULATOR"]:
        num_ind_var = 2
    else:
