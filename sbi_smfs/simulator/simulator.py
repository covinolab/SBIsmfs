...
from sbi_smfs.simulator.langevin_integrator import langevin_integrator
...
def smfe_simulator_langevin(
    ...
    # Select integration constants from parameters
    num_ind_params = 4
    xm = 10 ** parameters[0].item()
    xnu = 10 ** parameters[1].item()
    Dq = 10 ** parameters[2].item()
    k = 10 ** parameters[3].item()
...
def get_simulator_from_config(
    ...
    if "Dx" in config["SIMULATOR"]:
        ...
        Dx = None
    elif "type_xm" in config["PRIORS"] and "type_xnu" in config["PRIORS"] and "parameters_xm" in config["PRIORS"] and "parameters_xnu" in config["PRIORS"]:
        return partial(
            smfe_simulator_langevin,
