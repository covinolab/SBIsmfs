...
def test_langevin_integrator_error():
    deltaG = 6
    k = 3
    delta_x = 1.5
    x_knots = np.linspace(-6, 6, 150)
...
def test_langevin_integrator_saving():
    deltaG = 6
    k = 3
    delta_x = 1.5
    x_knots = np.linspace(-6, 6, 150)
...
@pytest.mark.parametrize("deltaG, k, delta_x", [(6, 3, 1.5), (4, 2, 1.0)])
def test_langevin_integrator_pmf(deltaG: float, k: float, delta_x: float):
    x_knots = np.linspace(-6, 6, 150)
    y_knots = deltaG * G0(x_knots / delta_x)
...
def test_langevin_integrator_rng_generator():
    deltaG = 6
    k = 3
    delta_x = 1.5
    x_knots = np.linspace(-6, 6, 150)
